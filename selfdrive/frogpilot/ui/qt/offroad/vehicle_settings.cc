...
QStringList getCarNames(const QString &carMake, QMap<QString, QString> &carModels) {
  ...
  QString filePath = QString("../car/%1/values.py").arg(targetFolder);
  QFile file(filePath);
  QStringList names;
  ...
  if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
    ...
    QRegularExpression carDocsClassRegex(R"delimiter(class (\w+CarDocs)\()delimiter");
    QRegularExpressionMatch carDocsClassMatch = carDocsClassRegex.match(fileContent);

    if (carDocsClassMatch.hasMatch()) {
      QString carDocsClassName = carDocsClassMatch.captured(1);

      QRegularExpression carModelRegex(R"delimiter((\w+)\s*=\s*(\w+PlatformConfig|PlatformConfig)\([\s\S]*?\[([^\]]+)\])delimiter");
      ...
      while (carModelIt.hasNext()) {
        ...
        QString carModel = carModelMatch.captured(1);
        QString carDocsArray = carModelMatch.captured(3);

        QRegularExpression carDocsRegex(QString(R"delimiter(%1\(["']([^"']+?)["'](?:, [^\)]*)?\))delimiter").arg(carDocsClassName));
        QRegularExpressionMatchIterator carDocsIt = carDocsRegex.globalMatch(carDocsArray);

        while (carDocsIt.hasNext()) {
          QRegularExpressionMatch carDocsMatch = carDocsIt.next();
          QString carName = carDocsMatch.captured(1);

          if (carName.contains(QRegularExpression("^[A-Za-z0-9 .-]+$")) && carName.count(" ") >= 1) {
            names << carName;
            carModels[carName] = carModel;
          }
        }
      }

      QRegularExpression footnoteRegex(QString(R"delimiter(%1\(["']([^"']+?)["'].*footnotes=\[.*\])delimiter").arg(carDocsClassName));
      QRegularExpressionMatchIterator footnoteIt = footnoteRegex.globalMatch(fileContent);

      while (footnoteIt.hasNext()) {
        QRegularExpressionMatch footnoteMatch = footnoteIt.next();
        QString carName = footnoteMatch.captured(1);

        if (carName.contains(QRegularExpression("^[A-Za-z0-9 .-]+$")) && carName.count(" ") >= 1) {
          names << carName;
          carModels[carName] = carModelRegex.pattern();
        }
      }
    }
  }
...
FrogPilotVehiclesPanel::FrogPilotVehiclesPanel(SettingsWindow *parent) : FrogPilotListWidget(parent) {
  ...
  QObject::connect(selectMakeButton, &ButtonControl::clicked, [this]() {
    QStringList makes = {
      "Acura", "Audi", "Buick", "Cadillac", "Chevrolet", "Chrysler", "Dodge", "Ford", "GM", "GMC",
  ...
  std::vector<std::tuple<QString, QString, QString, QString>> vehicleToggles {
    ...
    {"GasRegenCmd", tr("Truck Tune"), tr("Increase the acceleration and smoothen out the brake control when coming to a stop."), ""},
